...
app.get("/getall", async (req, res) => {
  ...
  try {
    // browser = await puppeteer.launch();
    const [am] = await Promise.all([amazonsearch(st)]);
...
const amazonsearch = async (str) => {
  ...
  const titles = await pagee.$$eval(
    "a.a-text-normal > span.a-text-normal",
    (items) => items.slice(0, 6).map((item) => item.textContent)
  );

  const titlesss = await pagee.$$eval(".a-price-whole", (items) =>
    items.slice(0, 6).map((item) => item.textContent)
  );
  const url = await pagee.$$eval(
    ".a-link-normal.s-underline-text.s-underline-link-text.s-link-style.a-text-normal",
    (items) =>
      items
        .slice(0, 6)
        .map((item) => "https://www.amazon.in" + item.getAttribute("href"))
  );
  const imgs = await pagee.$$eval(
    ".a-section.aok-relative.s-image-fixed-height > img",
    (items) => items.slice(0, 6).map((item) => item.getAttribute("src"))
  );
...
const flipkartsearch = async (str) => {
  ...
  const titles = await page.$$eval("._30jeq3 ", (items) =>
    items.slice(0, 6).map((item) => item.textContent)
  );
  let titless = await page.$$eval("._4rR01T", (items) =>
    items.slice(0, 6).map((item) => item.textContent)
  );
  let url = await page.$$eval("._1fQZEK", (items) =>
    items
      .slice(0, 6)
      .map((item) => "https://www.flipkart.com" + item.getAttribute("href"))
  );
  if (titless.length < 1) {
    titless = await page.$$eval(".s1Q9rs ", (items) =>
      items.slice(0, 6).map((item) => item.textContent)
    );
    url = await page.$$eval(".s1Q9rs", (items) =>
      items
        .slice(0, 6)
        .map((item) => "https://www.flipkart.com" + item.getAttribute("href"))
    );
  }
  const imgss = await page.$$eval("._396cs4 ", (items) =>
    items.slice(0, 6).map((item) => item.getAttribute("src"))
  );
  const finalarr = [];
  for (i = 0; i <= titles.length; i++) {
...
const cromasearch = async (str) => {
  ...
  try {
    const titles = await pagew.$$eval(".sp__name", (items) =>
      items.slice(0, 5).map((item) => item.textContent)
    );

    const titless = await pagew.$$eval(".gimCrs", (items) =>
      items.slice(0, 5).map((item) => item.textContent)
    );
    const imgs = await pagew.$$eval(
      "div.sp__productbox > div:nth-child(2) > div > img",
      (items) =>
        items
          .slice(0, 5)
          .map(
    ...
    let url = await pagew.$$eval(".sp.grid > a", (items) =>
      items
        .slice(0, 5)
        .map(
          (item) => "https://www.reliancedigital.in" + item.getAttribute("href")
        )
    );
    const finalarr = [];
    for (i = 0; i <= titles.length; i++) {
...
const shopcluessearch = async (str) => {
  ...
  const titles = await page.$$eval("a>h2", (items) =>
    items.slice(0, 5).map((item) => item.textContent)
  );
  const titless = await page.$$eval(".p_price", (items) =>
    items.slice(0, 5).map((item) => item.textContent)
  );
  const finalarr = [];
  for (i = 0; i <= titles.length; i++) {
